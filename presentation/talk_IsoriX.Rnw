...
\end{enumerate}
\vfill
A complete assignment can be done in 6 lines of code \hyperlink{all_in_6}{\beamerskipbutton{proof}}
...
To avoid troubles:
\begin{itemize}
\item use a \verb+data.frame+ (and not a \verb+tibble+)
...
<<check source data, fig.show = "hold", out.width = "4cm", out.height = "5cm", size = "tiny">>=
...
\item Weighted by precipitation or not? If yes, you have 2 possibilities, before or after fitting:
\begin{itemize}
...
\vfill
\pause
Note: $d_{ab}$ is (by default) measured accounting for the curvature of the Earth.
\end{frame}
...
\pause
\item If you compare models with different structures for the residual dispersion model, (for the time being) you must correct the cAIC of the mean model by adding to it twice the number of parameters of the residual dispersion model. Here this latter model has 5 parameters (1 intercept, 2 variances of random effects, 2 correlation parameters for the Matern). [Note: if residual dispersion models are the same then the correction won't change the ranking of the models]
\end{itemize}
\end{frame}
...
\item The test statistics is the difference between the predicted isoscape value at the origin location (deduced from the calibration fit) and the predicted isoscape value at the location under consideration (read from the isoscape).
...
An entire workflow can takes as little as 6 lines of code:
\bigskip
...
@
\vfill
Note: you should always check all intermediary outputs as we did for Germany.  \hyperlink{workflow}{\beamerskipbutton{}}
\end{frame}


\begin{frame}[t, fragile]{Where does the bat Nnoc\_15 comes from?}
<<all_in_one_plot, out.height = "8.5cm", out.width = "8.5cm">>=
...
\vfill
\item More predictors (e.g. temperature) with data download using \r\ packages.
\vfill
