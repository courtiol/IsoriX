...
\item as simple as necessary (but not too beyond that)
\item each step of the analysis can be thoroughly studied
\item cutting edge statistical methods accounting for many source of uncertainties
...
\item a little stable isotope knowledge required
