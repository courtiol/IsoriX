...
That is why we have already stopped relying on _rgdal_ and _Cairo_ which are not so easy to install on some systems.

Despite these effort, for IsoriX to work we do rely on `r length(miniCRAN::pkgDep("IsoriX", suggests = TRUE, recursive = FALSE)) - 1` packages, as illustrated here with the help of the package miniCRAN:
...
plot(miniCRAN::makeDepGraph("IsoriX", recursive = FALSE, suggests = TRUE))
```
...
All in all, this represents quite a lot of dependencies (although much less than many other packages wildly used), and problems can thus happen.

In case of problem(s), you should thus read carefully the error message and make sure that the problem is not caused by some other package. If that is the case, try to install the problematic package on its own (check the documentation of that other package, it may help). A work around is possible if the problematic package is one that IsoriX "suggests" but does not depend on (i.e. it could live without it and still perform all essential tasks without it); in that case, you may try to install IsoriX again using:

```{r install2, eval = FALSE}
install.packages("IsoriX")
```

that is, without specifically asking to install the extra dependencies. If you are used to install the packages using the RStudio menu, there is a small box that you can tick or untick to do the same thing. 
...
For example if you have 4 CPU you can do:
