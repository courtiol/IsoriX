...
#'       lambda.matern.disp = NewdatFit$disp_fit$lambda
#'     )
#'   ), 4)
#' }
#'
...
isosim <- function(data,
  ...
  if (inherits(data, "SpatRaster")) {
    ...
    data <- data.frame(
      ...
      source_ID = as.factor(paste("simu", seq_len(nrow(coord)), sep = "_"))
    )
    rm(coord)
  ...
  RandomFields::RFoptions(
    ...
    cPrintlevel = 1
  ) ## cPrintlevel = 3 for more details
  ...
  LinpredDisp <- .linear_predictor(
    fix_coef = disp_model_fix_coef,
  ...
  ## compute the linear predictor
  MeanLinpred <- .linear_predictor(
    fix_coef = mean_model_fix_coef,
