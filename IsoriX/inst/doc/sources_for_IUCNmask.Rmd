...
Because we want to illustrate the IUCN range of a species on top of an assignment map, we need to have proceeded thorough an entire IsoriX workflow including the assignment itself. To make this quick, we will simply replicate the minimal example given for the function `isofind()` to run an entire workflow:
...
```{r elevation, results='hide'}
## We prepare the elevation raster
...
assignment <- isofind(assign.data = subset(assigndata, species == "Myotis_bechsteinii"),
                      isoscape = isoscape,
                      calibfit = calib)
```
...
Now we have created a polygon that shows the IUCN range of our species in the same projection as our assignemnt.
...
```{r create mask}
library(rgeos)
limits <- as(extent(assignment$indiv$pv), "SpatialPolygons")
...
We can use the function `rgb()` to specify a color with an alpha transparency.
