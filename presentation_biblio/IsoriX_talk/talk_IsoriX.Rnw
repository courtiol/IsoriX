...
\newcommand{\frp}[1]{\textcolor{purple}{\textbf{#1}}}
\newcommand{\frm}[1]{\textcolor{red}{\st{#1}}}
\newcommand{\frc}[1]{\textcolor{blue}{{[FR:\ #1]}}}

\newcommand{\alexp}[1]{\textcolor{olive}{\textbf{#1}}}
\newcommand{\alexm}[1]{\textcolor{orange}{\st{#1}}}
\newcommand{\alexc}[1]{\textcolor{cyan}{{[Alex:\ #1]}}}

\newcommand{\todo}[1]{\textcolor{green}{{[ToDo:\ #1]}}}
\newcommand{\question}[1]{\textcolor{red}{{[\begin{Large}QUESTION:\end{Large}\ #1]}}}
...
\item simple enough (but not too simple) and highly customizable
\item compatible with Geographic Information System within and without \R
\item cutting edge statistical methods
...
\item not multivariate (1 isotope only)
...
An interface between \R \ packages (without help files: only \Sexpr{linesofcode} lines of codes):
\bigskip
...
Remember:
\begin{itemize}
\item Always try to be explicit about which sources of uncertainty are acounted for and which are not!
...
plot(CalibBats)
@
Note: this looks bad, but in fact it is increadibly good! Despite huge extrapolation, the fit is very close to a calibration fitted on an european isoscape (see \url{https://bookdown.org/content/782/calibration.html}). The reason why it works so well is that the uncertainty in $x$ is accounted for.
\end{frame}
...
\begin{frame}[t, fragile]{Step 6. Perform the assignment}
6b. Use \texttt{isofind} performs the assignment test:
...
<<>>=
names(myassignment)
names(myassignment$indiv)
myassignment$indiv$stat
@
\end{scriptsize}
\end{frame}
...
\begin{frame}[t, fragile]{Step 6. Perform the assignment}
\begin{scriptsize}
<<fig.width=10, fig.height=7, out.height="0.7\\textheight", fig.asp=0.5>>=
plot(x=myassignment, who="Mbe_10",
  borders=list(borders=worldcountries), mask=list(mask=oceanmask),
...
\begin{frame}[t, fragile]{Step 6. Perform the assignment}
\begin{scriptsize}
<<fig.width=10, fig.height=7, out.height="0.7\\textheight", fig.asp=0.5, cahce=FALSE>>=
assplot <- plot(x=myassignment, who="group",
  borders=list(borders=worldcountries), mask=list(mask=oceanmask),
  palette=isorixpalette)
@
\end{scriptsize}
...
\begin{frame}[t, fragile]{How good is the assignment?}
\begin{scriptsize}
<<fig.width=10, fig.height=7, out.height="0.7\\textheight", fig.asp=0.5>>=
## the real origin of these bats (as known by us)
origin <- data.frame(long=25.982122, lat=43.611536)
library(lattice)
assplot + xyplot(origin$lat~origin$long, pch=13, col="purple", cex=5, lwd=2,
         panel=panel.points)
@
\end{scriptsize}
\end{frame}

\begin{frame}[t, fragile]{Why can assignment fail?}
1. Because individuals may come from different places:
\begin{scriptsize}
<<fig.width=10, fig.height=7, out.height="0.7\\textheight", fig.asp=0.5>>=
plot(x=myassignment, who=1:9,
  borders=list(borders=worldcountries), mask=list(mask=oceanmask),
  sources=list(draw=FALSE), calib= list(draw=FALSE), palette=isorixpalette)
@
\end{scriptsize}
\end{frame}

\begin{frame}[t, fragile]{Why can assignment fail?}
2. Because calibration may be poor:
\begin{scriptsize}
<<>>=
test <- data.frame(env=extract(myisoscape$Isoscape$mean, origin),
           tissue=M_bechsteinii$tissue.value)
@
<<eval=FALSE>>=
test
@
...
<<eval=FALSE>>=
plot(mycalib)
points(test$tissue~test$env, col="red")
@
<<fig.width=6, fig.height=5.5, out.height="0.6\\textheight", out.width="0.7\\textheight", echo=FALSE>>=
par(mar=c(4,4,0.1,1))
plot(mycalib)
points(test$tissue~test$env, col="red")
@
...
\begin{frame}[t, fragile]{Why can assignment fail?}
3. Because the isoscape may be poor:
\begin{scriptsize}
<<fig.width=10, fig.height=7, out.height="0.6\\textheight", out.width="0.8\\textheight">>=
## the location with the largest p-value
coord <- coordinates(myassignment$group$pv)
max.location <- coord[which.max(values(myassignment$group$pv)), ]
maximum <- data.frame(long=max.location[1], lat=max.location[2])
assplot + xyplot(maximum$lat~maximum$long, pch=13, col="orange", cex=5, lwd=2,
         panel=panel.points)
@
\end{scriptsize}
\end{frame}
...
\begin{scriptsize}
<<fig.width=10, fig.height=7, out.height="0.6\\textheight", out.width="0.8\\textheight">>=
predVarplot <- plot(myisoscape, which="mean.predVar",
                    borders = list(borders = worldcountries),
                    mask = list(mask = oceanmask),
                    palette = isoscapepalette, plot=FALSE)
predVarplot + xyplot(maximum$lat~maximum$long, pch=13, col="orange", cex=5, lwd=2,
         panel=panel.points)
@
\end{scriptsize}
\end{frame}
...
\begin{frame}{Life of IsoriX}
Current versions:
\begin{small}
\begin{itemize}
\item IsoriX 0.4-1: stable version (1$^\text{st}$ submission to CRAN, 11/09/2016)
\item IsoriX 0.4-2: development version
\end{itemize}
\end{small}
\vfill
Soon:
\begin{small}
\begin{itemize}
\item more tutorials and better help files
\item supporting publications
\item optional calibration
\item 3D plots
\item map projections
\item finer statistical methods (DHGLM)
\end{itemize}
\end{small}
\end{frame}

\begin{frame}{To follow the development of IsoriX}
\centerline{
