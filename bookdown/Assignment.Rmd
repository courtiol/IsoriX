...
*The null hypothesis of this group assignment is that all assignment samples originated from a location with identical distribution of source isotope values as the one being tested.*
Therefore, the test considers that all assignment samples must come from the same location of origin (or at least from a set of locations presenting the same isotopic composition), and if this assumption is not fulfilled, the test may exclude all candidate locations.
This is why you should always start by exploring individual assignments even you are ultimately interested in a group assignment.
...
By default, a 95%-confidence region is being displayed in grey, which means that the non-grey areas should contain the true location of origin with a 95% probability.
...
You may want to use it, for exampple, to hide areas outside the IUCN distribution map for your species.
