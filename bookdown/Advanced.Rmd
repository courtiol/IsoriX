...
Yet, the instructions that follow could also be adapted to other type of isoscapes that must be weighed for other reasons.
...
Here the weights we will use are the monthly cumulative amount of precipitation water averaged over the years, but you could also adapt the procedure to use a different temporal resolution.
...
We start by aggregating and weighting the data per year:
...
We then use aggregate the data, for each location, by computing averages over the different months.
...
Let us now compare the mean isoscapes produced with and without weighing by precipitation amount:
...
As you can see the residual variance is quite different because it now only represents the yearly averaged weighted variation in isotopic values _between months_ and no longer the total variation combining both the variation across months and across years.
