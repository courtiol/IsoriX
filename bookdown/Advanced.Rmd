...
If lattice is not for you and are more familiar with [**ggplot2**](https://ggplot2.tidyverse.org/index.html), you can use it.
We did not implement specific functions for doing so, but since IsoriX spatial objects are now made with [**terra**](https://rspatial.github.io/terra/), it is relativelly straightforward.

Here, is a first example using the recent pacakge [**tidyterra**](https://dieghernan.github.io/tidyterra/) which creates a plot of the mean isoscape:
...
For several applications it is often recommend to produce *weighted isoscapes*.
...
Biologically, taking the context of assignment of animals based on isoscapes derived from precipitation water, we would therefore want this variance to represent the variation between water sample drank by an animal within the local area where the animal grew the tissue used for measurements.
...
Those uncertainties explain why we have not yet fully automatised the computation in IsoriX.
...
Note that since the variance represents variation between years, the locations for which a single year of data were discared.
...
As you can see the residual variance is considerable smaller for the precipitation-weighted annual average isoscape than for the non-weighted isoscape.
This is because the residual variance now represents the variation in isotopic values _between years_ and no longer the total variation combining both the variation across years and months.
