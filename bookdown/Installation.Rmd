...
The stable version doesn’t change often, but as the name suggests, it should be reliable and free of major bugs.
...
The list of such packages that are not crucial for `IsoriX` (but that offer additional functionalities) is this: *`r sort(setdiff(miniCRAN::pkgDep("IsoriX", suggests = TRUE), miniCRAN::pkgDep("IsoriX", suggests = FALSE)))`*.
